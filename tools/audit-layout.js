 ...
 */
(function(){
  /* NOTHING MEANINGFUL COMES OUT OF A COLLAPSED VIEWPORT.

     Measured once, by accident: the pane lost its size, innerWidth went to 0,
     and the sweep reported 10 overlaps, 13 bleeds and "grid tracks all ~0" on
     every tab in both editions. Every box is zero wide, so every box intersects
     every other one and every track has collapsed - all true, all meaningless.

     Refused rather than annotated, because the failure mode is that somebody
     believes it. The findings are specific, plausible and numerous, and the one
     number that gives it away is the one a report like that never prints. */
  if (!window.innerWidth || window.innerWidth < 320 || !window.innerHeight){
    return {
      refused: "viewport is " + window.innerWidth + "x" + window.innerHeight +
               " - too small to measure; nothing below 320px wide is real",
      counts: {}, total: 0
    };
  }
